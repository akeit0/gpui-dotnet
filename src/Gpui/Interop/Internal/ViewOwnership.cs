...
internal sealed class ViewOwnership
{
    ...
    [ThreadStatic] internal static bool Constructing;
    ...
    internal void Bind(ViewBase view)
    {
        ...
        if (View is not null || ConstructionComplete)
            throw new InvalidOperationException("A construction context belongs to exactly one new View.");
        View = view;
    }
    ...
    internal void AssertAccess()
    {
        if (_thread != Environment.CurrentManagedThreadId)
            throw new InvalidOperationException("Owned View facilities require the application thread.");
        if (_execution is not null && ApplicationExecution.Current is { } current && !ReferenceEquals(_execution, current))
            throw new InvalidOperationException("An owned facility cannot be used by another application.");
    ...
    internal Memo<TInput, TResult> Memo<TInput, TResult>() where TInput : IEquatable<TInput>
    {
        AssertAccess();
    ...
    internal Effect<TInput> Effect<TInput>(Action<EffectScope, TInput> setup) where TInput : IEquatable<TInput>
    {
        AssertAccess();
    ...
    internal void ClearCaches()
    {
        ...
        if (Effects is not null)
            foreach (var effect in Effects) effect.CodeChanged();
    }
    ...
    internal void Retire()
    {
        ...
        if (Effects is not null)
            for (var i = Effects.Count - 1; i >= 0; i--)
                try { Effects[i].Retire(); } catch (Exception e) { (failures ??= []).Add(e); }
        Effects = null;
        if (_caches is not null)
            foreach (var cache in _caches) cache.Clear();
        _caches = null;
    ...
    {
        if (Effects is not null)
            foreach (var effect in Effects) effect.Revoke();
    }
}
