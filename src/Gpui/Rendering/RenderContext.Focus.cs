...
public readonly unsafe ref partial struct RenderContext
{
    ...
    /// </summary>
    public Element<DivTag> FocusTarget(ref FocusController controller, Element<DivTag> element,
        bool tabStop = false)
    {
        ...
        if (controller.IsBound && !ReferenceEquals(controller.Owner, owner))
            throw new InvalidOperationException("A focus controller must be declared by its owning View.");
        if (!controller.IsBound)
            controller = new FocusController(owner, ResourceKeys.EncodeAutoKey(owner.Runtime.NextResourceKeyId()));
