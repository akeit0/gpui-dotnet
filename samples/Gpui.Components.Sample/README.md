...
The sample builds the optional component host with its `editor` feature, including the gpui-kit
catalog and Editor. It demonstrates display elements, controlled inputs, file presentation, a
conversation, and retained editing. The empty state uses the sample's `Assets/archive-box.svg`
file through the native Icon component's application asset path. The Save button uses an icon from
the host's bundled assets.
