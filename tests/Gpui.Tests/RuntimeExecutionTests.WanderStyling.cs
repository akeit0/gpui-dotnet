...
public sealed unsafe partial class RuntimeExecutionTests
{
    ...
    public void WanderTabsInheritNavigationForegroundAcrossSelectionAndThemeChanges()
    {
        ...
        using var fixture = new SessionFixture(null, application,
            new RootViewDeclaration<Travel.WanderShellView>(spec), window);

        foreach (var theme in new[] { Travel.WanderThemes.Light, Travel.WanderThemes.Dark, Travel.WanderThemes.Light })
        {
            ...
            for (var selected = 0; selected < 4; selected++)
            {
                ...
                var children = new ReadOnlySpan<ChildRecord>(arena.Children, arena.ChildLength).ToArray();
                for (var tab = 0; tab < 4; tab++)
                {
                    var key = System.Text.Encoding.UTF8.GetBytes($"tab-{tab}");
                    var root = Enumerable.Range(0, arena.NodeLength).Single(index =>
                        arena.Nodes[index].Component == (ushort)ComponentId.Button &&
                        new ReadOnlySpan<byte>(arena.Utf8 + arena.Nodes[index].DataOffset,
                            (int)arena.Nodes[index].DataLength).SequenceEqual(key));
                    ...
                    Assert.Equal(3, descendants.Count(id => arena.Nodes[id].Component == (ushort)ComponentId.Text));
                    Assert.DoesNotContain(ops, op => descendants.Contains(op.Node) && op.Code == (ushort)OpCode.TextRgba);
                    var foreground = new Color((uint)ops.Last(op => op.Node == root && op.Code == (ushort)OpCode.TextRgba).A);
                    var background = new Color((uint)ops.Last(op => op.Node == root && op.Code == (ushort)OpCode.BackgroundRgba).A);
                    var expected = Travel.WanderStyles.Button(theme, Travel.WanderButtonVariant.Navigation, tab == selected);
