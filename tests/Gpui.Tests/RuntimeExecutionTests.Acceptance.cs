...
public sealed partial class RuntimeExecutionTests
{
    ...
    public void CleanBranchAcceptsEqualPropsWithoutRedeclaringItsDescendants()
    {
        ...
        fixture.Render();
        var branch = Assert.IsType<AcceptancePropsBranch>(fixture.State(fixture.View).Children!.Values.Single().View);
        var leaf = Assert.IsType<PropsChildView>(fixture.State(branch).Children!.Values.Single().View);
        var initial = props;
    ...
    public void ReplacingBranchRetiresItsWholeSubtreeBeforeActivation(bool cleanupFails)
    {
        ...
        using var fixture = new SessionFixture(new DeclarationRoot((ref RenderContext ui) =>
            ui.Div(
                replace ? ui.Child("replace", ChildView.Spec()) : ui.Child("replace", BranchView.Spec()),
                ui.Child("keep", BranchView.Spec()))));
        fixture.Render();
        var branches = fixture.State(fixture.View).Children!.Values.Select(entry => (BranchView)entry.View).ToArray();
        var removed = branches[0];
        var retained = branches[1];
        var leaves = fixture.State(removed).Children!.Values.Select(entry => (ChildView)entry.View).ToArray();
        var retainedLeaves = fixture.State(retained).Children!.Values.Select(entry => (ChildView)entry.View).ToArray();
        ...
        replace = true;
        fixture.Publish();
        var replacement = fixture.State(fixture.View).StagedChildren!.Values.Select(entry => entry.View).OfType<ChildView>().Single();
        ...
        Assert.All(retainedLeaves, leaf =>
        {
            Assert.Equal(1, leaf.RenderCount);
        ...
        {
            fixture.Render();
            Assert.Same(retained, fixture.State(fixture.View).Children!.Values.Select(entry => entry.View).OfType<BranchView>().Single());
    ...
    public void RetainedAcceptanceCost(int branches, bool dirtyLeaf)
    {
        ...
        {
            long publishTicks = 0, acceptTicks = 0, allocated = 0;
            for (var iteration = 0; iteration < iterations; iteration++)
            {
                if (dirtyLeaf) leaf.Invalidate();
    ...
    private sealed class AcceptanceBranch(ViewConstruction construction)
        : View<int>(construction), IGeneratedViewFactory<AcceptanceBranch, int>
    {
        public static AcceptanceBranch CreateGpuiView(ViewConstruction construction, int props) => new(construction);

        protected override Element Render(in int props, ref RenderContext ui)
        {
            var root = ui.Div();
            for (var index = 0; index < props; index++) root.Child(ui.Child(ChildView.Spec()));
            return root;
        }
    }

    private sealed class AcceptancePropsBranch(ViewConstruction construction)
        : View<LabelProps>(construction), IGeneratedViewFactory<AcceptancePropsBranch, LabelProps>
    {
        public static AcceptancePropsBranch CreateGpuiView(ViewConstruction construction, LabelProps props) => new(construction);
