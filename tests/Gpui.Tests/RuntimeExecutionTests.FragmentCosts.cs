...
public sealed unsafe partial class RuntimeExecutionTests
{
    ...
    public void RetainedFragmentCopyAndValidationCost(int leaves)
    {
        ...
        var copiedBytes =
            input->NodeLength * sizeof(NodeRecord)
            + input->OpLength * sizeof(OpRecord)
            + input->ChildLength * sizeof(ChildRecord)
            + input->Utf8Length;
        var capacityBytes =
            output->NodeCapacity * sizeof(NodeRecord)
            + output->OpCapacity * sizeof(OpRecord)
            + output->ChildCapacity * sizeof(ChildRecord)
            + output->Utf8Capacity;
