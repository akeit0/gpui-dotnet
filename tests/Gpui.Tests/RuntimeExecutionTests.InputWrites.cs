...
public sealed unsafe partial class RuntimeExecutionTests
{
    ...
    public void InputWriteRequestsPreserveIdsAndPoliciesAcrossAnyThreadIngress(bool utf8)
    {
        ...
        var thread = new Thread(() =>
            ...
            {
                if (utf8)
                    controller.SetValueIfCurrentWithResult(
                        "日本語"u8,
                        19,
                        ulong.MaxValue >> 2,
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            controller.SetValueIfCurrentWithResult("", 0, 1)
        );
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            controller.SetValueIfCurrentWithResult("", 1, 0)
        );
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            controller.SetValueIfCurrentWithResult("", 1, 1UL << 62)
        );
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            controller.SetValueIfCurrentWithResult("", 1, 1, (InputSelectionPolicy)2)
        );
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            controller.SetValueIfCurrentWithResult("", 1, 1, composition: (InputCompositionPolicy)2)
        );
        Assert.Single(calls);
    }
    ...
    public void InputWriteResultsUseTypedBindingsAndRetireWhenTheBindingIsRemoved(
    ...
    )
    {
        ...
        var token = new ReadOnlySpan<OpRecord>(arena.Ops, arena.OpLength)
            .ToArray()
            .Single(op => op.Code == (ushort)OpCode.InputOnWriteCompleted)
            .A;
        ...
        Assert.Equal(
            ...
            fixture.Control(
                token,
                (ushort)InputWriteEventKind.Completed,
                bytes,
                revision: ulong.MaxValue
            )
        );
        Assert.Equal(
            new InputWriteResult(ulong.MaxValue >> 2, outcome, ulong.MaxValue),
            view.Result
        );
    ...
    public void InputWriteResultsRejectMalformedPacketsWithoutInvokingTheHandler()
    {
        ...
        foreach (
            var payload in new[]
            {
                WriteResultPayload(0, 0),
                WriteResultPayload(1UL << 62, 0),
                WriteResultPayload(1, 4),
                WriteResultPayload(1, 0)[..15],
            }
        )
            Assert.Equal(
                -112,
                fixture.Control(token, (ushort)InputWriteEventKind.Completed, payload, revision: 1)
            );
        ...
        Assert.Equal(
            -112,
            fixture.Control(token, (ushort)InputWriteEventKind.Completed, valid, revision: 1)
        );
        Assert.Null(view.Result);
        Assert.Null(fixture.Session.Failure);
    }
    ...
    private sealed class InputWriteProbe : ProbeView
    {
        ...
        protected override Element Render(ref RenderContext ui)
        {
            ...
            return Observe
                ? input.OnWriteCompleted(this, static (view, result) => view.Result = result)
                : input;
        }
    }
}
