...
public sealed partial class RuntimeExecutionTests
{
    ...
    public void WorkCancellationIsDeferredAndDoesNotFaultTheWindow(string pattern, bool handled)
    {
        ...
        var operation = scope.StartCore(calls, (pattern, source.Task), static (request, _) =>
    ...
    public void FaultedTaskWithCancellationExceptionIsStillAFailure()
    {
        ...
        scope.Start(observed, Task.FromException<int>(failure), static (task, _) => task,
    ...
    public void FaultedTaskDeliversFirstExceptionWithoutRethrowingItDuringObservation()
    {
        ...
        var observed = new List<Exception>();
        fixture.View.Runtime.GetWorkScope().Start(observed, source.Task, static (task, _) => task,
            static (_, _) => Assert.Fail("Unexpected success"),
            static (state, error) => state.Add(error));
        fixture.Render();
    ...
    private sealed class ReplaceableLoad
    {
        ...
        internal WorkScope.PendingWork Start(WorkScope scope, Task<int> task, CancellationToken cancellation)
        {
            Busy = true;
            return scope.StartCore((load: this, revision: ++_revision), (task, cancellation),
                static async (request, lifetime) =>
                {
                    using var linked = CancellationTokenSource.CreateLinkedTokenSource(lifetime, request.cancellation);
    ...
    public void CancellationHandlerFailureFaultsIngress()
    {
        ...
        fixture.View.Runtime.GetWorkScope().Start(failure, Task.FromCanceled<int>(new(true)),
            static (task, _) => task, static (_, _) => Assert.Fail("Unexpected success"),
    ...
    public void RetirementDropsAnAlreadyQueuedCancellation()
    {
        ...
        var calls = new List<string>();
        var source = new TaskCompletionSource<int>();
        var operation = fixture.Child.Runtime.GetWorkScope().StartCore(calls, source.Task,
            static (task, _) => task, static (_, _) => Assert.Fail("Unexpected success"),
