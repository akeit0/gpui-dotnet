...
public sealed unsafe partial class RuntimeExecutionTests
{
    ...
    public void ItemTooltipRequestDecodesIdentityAndPlacementOutsideTheItem()
    {
        ...
        Assert.Equal(
            700UL,
            Assert.Single(ops, op => op.Code == (ushort)OpCode.TooltipShowDelayMs).A
        );
        Assert.Equal(
            650UL,
            Assert.Single(ops, op => op.Code == (ushort)OpCode.TooltipHideDelayMs).A
        );
        Assert.Equal(
            14f,
            BitConverter.UInt32BitsToSingle(
                (uint)Assert.Single(ops, op => op.Code == (ushort)OpCode.TooltipGapPx).A
            )
        );
        Assert.Equal(
            12f,
            BitConverter.UInt32BitsToSingle(
                (uint)Assert.Single(ops, op => op.Code == (ushort)OpCode.TooltipMarginPx).A
            )
        );
        var collection = Assert
            .Single(ops, op => op.Code == (ushort)OpCode.ListOnTooltipRequested)
            .Node;
        Assert.All(
            ops.Where(op =>
                op.Code
                    is (ushort)OpCode.TooltipPlacement
                        or (ushort)OpCode.TooltipAlignment
                        or (ushort)OpCode.TooltipShowDelayMs
                        or (ushort)OpCode.TooltipHideDelayMs
                        or (ushort)OpCode.TooltipGapPx
                        or (ushort)OpCode.TooltipMarginPx
            ),
            op => Assert.Equal(collection, op.Node)
        );
    ...
    public void TaskBoardTooltipUsesTaskIdentityAndPreservesSelection()
    {
        ...
        var token = Assert
            .Single(
                ...
                op => op.Code == (ushort)OpCode.ListOnTooltipRequested
            )
            .A;
        Assert.Equal(0, fixture.Complete(revision));
            ...
            var list = ui.List(ref _list, new ListDataSource(100, 7), BindListRenderer(1))
                .OnTooltipRequested(
                    ...
                    new TooltipOptions(
                        ...
                        hideDelay: TimeSpan.FromMilliseconds(650),
                        gap: 14,
                        margin: 12
                    )
                );
