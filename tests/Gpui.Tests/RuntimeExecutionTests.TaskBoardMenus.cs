    ...
    public void TaskBoardContextMenuActsOnClickedTaskAndPreservesSelection(string action)
    {
        ...
        var select = Assert
            .Single(
                ...
                op => op.Code == (ushort)OpCode.ListOnSelectionRequested
            )
            .A;
        ...
        Assert.Equal(
            ...
            fixture.Control(
                ...
                SelectionPayload(0, (ulong)selected.Id),
                2,
                1
            )
        );
        ...
        var request = Assert
            .Single(
                new ReadOnlySpan<OpRecord>(arena.Ops, arena.OpLength).ToArray(),
                op => op.Code == (ushort)OpCode.ListOnContextMenuRequested
            )
            .A;
        Assert.Equal(0, fixture.Complete(revision));
        Assert.Equal(
        ...
        else
        {
            ...
            {
                case "ctx-open":
                    var windows = PrivateSampleField<Dictionary<ulong, GpuiWindow>>(
                        application,
                        "_windows"
                    );
